...
const WhiteDesktopSlider: FC<IWhiteDesktopSliderProps> = ({ slides, showTitle = false }) => {
  return (
    <Container maxW={CONTAINER_MAX_WIDTH} display={{ base: 'none', md: 'block' }}>
      {showTitle && (
        <Text variant="cursive" fontSize={{ base: '2xl', md: '4xl', lg: '5xl', xl: '6xl' }}>
          News
        </Text>
      )}
            ...
            <Box key={i}>
              <Flex
                ...
                <Stack gap={{ md: 0, lg: 2, xl: 4 }} flex="1">
                  <Text fontSize={{ md: 'sm', lg: 'md', xl: 'xl' }}>{slide.date}</Text>
                  <Heading
                    color="black.500"
                  ...
                  <Text fontSize={{ md: 'xs', lg: 'sm', xl: 'md' }}>{slide.text}</Text>

                  <Box>
                    <Button variant="outline" size={{ md: 'sm', lg: 'sm', xl: 'md' }}>
                      Mehr anzeigen
                    </Button>
                  </Box>
                </Stack>
